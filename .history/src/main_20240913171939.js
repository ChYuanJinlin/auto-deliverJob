 ...
 * @LastEditTime: 2024-09-12 16:46:31
 * @FilePath: \yjl-blog\auto-deliver\src\main.js
 * @Description:
 *
...
class Boss {
        ...
        async function sendObserver() {
          ...
          const observer = new IntersectionObserver((entries, observer) => {
            entries.forEach((entry) => {
              if (entry.isIntersecting && !clickQueue.includes(entry.target)) {
  ...
  async selectTab() {
    ...
    console.log("url--", url);
    console.log("列表的总数0", this.data.length);
    Promise.resolve();
  }
  // 去沟通
