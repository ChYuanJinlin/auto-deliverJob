 ...
 * @LastEditTime: 2024-09-22 18:11:46
              ...
              runObserver() {
                ...
                observe = new IntersectionObserver((entries, observer) => {
                  entries.forEach(async (entry) => {
                    const element =
                      entry.target.firstChild.children[1]?.children[2]
                        ?.children[2];
                    ...
                    setTimeout(() => {
                      console.log('entry.target.firstChild.children[1]?.children[2]',entry.target.firstChild.children[1]?.children[2]);
          ...
          async function processQueue(observer) {
            ...
            (async function processNextItem() {
              ...
              if (item) {
                ...
                // 删除不匹配的岗位
                const element =
                  item.firstChild.children[1]?.children[2]?.children[2];
                // 触发 mouseenter 事件
                var event = new MouseEvent("mouseenter", {
                  view: window,
                  bubbles: true, // mouseenter 不冒泡，但这个参数在这里是无关紧要的
